...
server.tool(
  ...
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      type: 'x_post', requestId, content: args.content,
...
server.tool(
  ...
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      type: 'x_like', requestId, tweetUrl: args.tweet_url,
      groupFolder, timestamp: new Date().toISOString(),
    });
    const result = await waitForXResult(requestId);
    return { content: [{ type: 'text' as const, text: result.message }], isError: !result.success };
...
server.tool(
  ...
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      type: 'x_reply', requestId, tweetUrl: args.tweet_url, content: args.content,
      groupFolder, timestamp: new Date().toISOString(),
    });
    const result = await waitForXResult(requestId);
    return { content: [{ type: 'text' as const, text: result.message }], isError: !result.success };
...
server.tool(
  ...
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      type: 'x_retweet', requestId, tweetUrl: args.tweet_url,
      groupFolder, timestamp: new Date().toISOString(),
    });
    const result = await waitForXResult(requestId);
    return { content: [{ type: 'text' as const, text: result.message }], isError: !result.success };
...
server.tool(
  ...
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      type: 'x_quote', requestId, tweetUrl: args.tweet_url, comment: args.comment,
      groupFolder, timestamp: new Date().toISOString(),
    });
    const result = await waitForXResult(requestId);
    return { content: [{ type: 'text' as const, text: result.message }], isError: !result.success };
...
server.tool(
  'x_read',
  'Read a tweet or full thread from X (Twitter). Main group only. Navigates to the tweet and extracts the full thread by the original poster.',
  {
    tweet_url: z.string().describe('The tweet URL (e.g., https://x.com/user/status/123) or tweet ID'),
  },
  async (args) => {
    ...
    writeIpcFile(TASKS_DIR, {
      type: 'x_read', requestId, tweetUrl: args.tweet_url,
      groupFolder, timestamp: new Date().toISOString(),
    });
